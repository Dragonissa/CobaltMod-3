...
public class CMSpreadingBlock extends SpreadingSnowyDirtBlock {
	protected CMSpreadingBlock(Properties properties) {
		super(properties);
	}
	...
	public void randomTick(BlockState blockState, ServerLevel serverLevel, BlockPos blockPos, RandomSource randomSource) {
		if (!canBeGrass(blockState, serverLevel, blockPos)) {
			serverLevel.setBlockAndUpdate(blockPos, CMBlocks.COBALT_DIRT.defaultBlockState());
		} else {
			if (serverLevel.getMaxLocalRawBrightness(blockPos.above()) >= 9) {
				...
				for(int i = 0; i < 4; ++i) {
					...
					if (serverLevel.getBlockState(blockPos2).is(CMBlocks.COBALT_DIRT) && canPropagate(blockState2, serverLevel, blockPos2)) {
